...
class MonetModel:
    ...
    def transfer_labels(
            self, ref_matrix: ExpMatrix, ref_cell_labels: pd.Series,
            target_matrix: ExpMatrix,
            num_neighbors: int = 20, **kwargs) -> pd.Series:
        
        le = LabelEncoder()
        
        ref_pc_scores = self.transform(ref_matrix)
        ...
        target_pc_scores = self.transform(target_matrix)
        ...
        if self.agg_num_neighbors_ == 1:
            ...
            for agg_step in range(self.num_aggregation_steps):
                ...
                _LOGGER.info('Now performing aggregation step %d/%d...',
                            agg_step, self.num_aggregation_steps)
