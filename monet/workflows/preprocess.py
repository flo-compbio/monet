...
def preprocess_10x_v3ts(
        cellranger_file: str, ensembl_annotation_file: str,
        expression_output_file: str = None, antibody_output_file: str = None,
        prefix='filtered_feature_bc_matrix',
        *args, **kwargs) -> Tuple[ExpMatrix, ExpMatrix, go.Figure]:
    """Preprocess data generated by 10x Genomics' CellRanger v3 pipeline."""
    ...
    sel = matrix.index.str.startswith('ENSG')
    antibody_matrix = matrix.loc[~sel].copy()
    matrix = matrix.loc[sel].copy()
    gc.collect()
    
    matrix, fig = preprocess_matrix(
        matrix, ensembl_annotation_file, expression_output_file,
        *args, **kwargs)

    antibody_matrix = antibody_matrix.loc[:, matrix.cells]

    if antibody_output_file is not None:
        # write antibody matrix to disk
        antibody_matrix.save(antibody_output_file)

    return matrix, antibody_matrix, fig
