...
def correct_mnn(
    ...
    performed after projecting the data into a latent space defined by a Monet
    model.
    => PMID: 29608177
    ...
    2. The PC scores obtained by applying the Monet model to the
       reference matrix.
    """
    ...
    # transform data using Monet model
    ref_pc_scores = monet_model.transform(ref_matrix)
    target_pc_scores = monet_model.transform(target_matrix)
    ...
    corr_vectors = np.empty((mnn.shape[0], monet_model.num_components_),
                            dtype=np.float32)
